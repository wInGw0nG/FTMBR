...
Func saveConfig() ;Saves the controls settings to the config
	...
	; If GUICtrlRead($chkRemainTrain) = $GUI_CHECKED Then
		; IniWrite($config, "troop", "CloseWhenTraining", 1)
	; Else
		; IniWrite($config, "troop", "CloseWhenTraining", 0)
	; EndIf

	; If GUICtrlRead($chkRemainNight) = $GUI_CHECKED Then
		; IniWrite($config, "troop", "CloseAtNight", 1)
	; Else
		; IniWrite($config, "troop", "CloseAtNight", 0)
	; EndIf
