...
$sModversion = " Doc Octopus v0.2.0.2 "
...
Func runBot() ; Bot that runs everything in order
	Local Static $aQueue[0][0]
	Local $aQueueActions[18][2] = [[$eCollect, $iDelayRunBot1], _
								   [$eCheckTombs, $iDelayRunBot3], _
								   [$eReArm, $iDelayRunBot3], _
								   [$eReplayShare, $iDelayRunBot3], _
								   [$eNotify, $iDelayRunBot3], _
								   [$eDonateCC, $iDelayRunBot1], _
								   [$eRequestCC, $iDelayRunBot1], _
								   [$eTrain, $iDelayRunBot1], _
								   [$eBoostBarracks, -1], _
								   [$eBoostSpellFactory, -1], _
								   [$eBoostDESpellFactory, -1], _
								   [$eBoostKing, -1], _
								   [$eBoostQueen, -1], _
								   [$eBoostWarden, -1], _
								   [$eResearch, $iDelayRunBot3], _
								   [$eUpgradeBuilding, $iDelayRunBot3], _
								   [$eUpgradeWalls, $iDelayRunBot3], _
								   [$eUpgradeHeroes, $iDelayRunBot3]]
	...
	While 1
		...
		ElseIf $RunState Then
			...
			If $Is_ClientSyncError = False And $Is_SearchLimit = False Then
				...
				; Check if there is any sequences left in the queue, if not create queue
				If UBound($aQueue) = 0 Then createQueue($aQueue, $aQueueActions)

				; Loop through all actions in the queue until the queue is empty
				Do
					; Check to see if we need to check the Android Time Lag
					Switch $aQueue[0][0]
						Case $eDonateCC, $eTrain, $eBoostBarracks
							If checkAndroidTimeLag() = True Then ContinueLoop
						Case Else
					...
					; Check to see if we need to recheck the main screen
					Switch $aQueue[0][0]
						Case $eDonateCC, $eRequestCC, $eTrain, $eResearch
...
Func Idle() ; Sequence that runs until Full Army
	Local Static $aQueue[0][0]
	Local $aQueueActions[4][2] = [[$eCleanYard, -1], _
								  [$eCollect, $iDelayIdle1], _
								  [$eReplayShare, $iDelayIdle1], _
								  [$eDonateCC, $iDelayIdle1]]
		...
		ElseIf $RunState Then
			...
			If $CommandStop = -1 Then
				...
				DropTrophy()
					...
					checkMainScreen(False)
				If _Sleep($iDelayIdle1) Then Return
			ElseIf $CommandStop = 0 And $bTrainEnabled = True Then
